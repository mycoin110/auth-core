 ...
 */
export function createPermissionGate(config: AuthCoreConfig, db: BetterSqlite3.Database): MiddlewareHandler {
  // 未启用权限系统 → 放行
  ...
  return async (c, next) => {
    ...
    const userModules = db
      .prepare<[number], { module_key: string }>(
        "SELECT module_key FROM user_modules WHERE user_id = ?",
      )
      .all(user.id) as { module_key: string }[];
