...
export function createAuthRoutes(config: AuthCoreConfig, db: import("better-sqlite3").Database) {
  const app = new Hono();
  ...
  app.post("/register", async (c) => {
    ...
    db.prepare(
      `INSERT INTO users (email, password_hash, status, role, display_name, created_at)
       VALUES (?, ?, 'pending', 'user', ?, ?)`,
    ).run(normalized, hash, cleanedDisplayName, now);
  ...
  app.get("/me", (c) => {
    ...
    if (!found) return c.json({ error: "会话失效" }, 401);
