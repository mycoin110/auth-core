...
// ==================== 登录页面 ====================
function loginHtml(config: AuthCoreConfig): string {
...
// ==================== 注册页面 ====================
function registerHtml(config: AuthCoreConfig): string {
  const pc = config.primaryColor ?? "#3b82f6";
  return `<!DOCTYPE html>
<html lang="zh-CN">
<head>
...
// ==================== 管理页面 ====================
function adminHtml(config: AuthCoreConfig): string {
  const pc = config.primaryColor ?? "#3b82f6";
  const rgb = hexToRgb(pc);
 ...
 * 适合插入到页面 header 的右上角。
 */
export function userWidgetHtml(config: AuthCoreConfig): string {
  const pc = config.primaryColor ?? "#3b82f6";
  return `
<style>
 ...
 */
export function userWidgetScript(config: AuthCoreConfig): string {
