 ...
 */
export function createAdminUserRoutes(config: AuthCoreConfig, db: import("better-sqlite3").Database) {
  const app = new Hono();
  ...
  app.get("/users", (c) => {
    const rows = db
      .prepare(
        `SELECT id, email, status, role, display_name, created_at, last_login_at
         FROM users
         ORDER BY
           CASE status
             WHEN 'pending'  THEN 0
      ...
      .all() as (Pick<UserRow, "id" | "email" | "status" | "role" | "display_name" | "created_at" | "last_login_at">)[];
  ...
  app.post("/users/:id/disable", (c) => {
    ...
    const me = (c as any).get("user") as UserRow | undefined;
    if (id === me?.id) return c.json({ error: "不能禁用自己" }, 400);

    const target = getUser(id);
    if (!target) return c.json({ error: "用户不存在" }, 404);
    if (target.status === "disabled") return c.json({ error: "该用户已是禁用状态" }, 400);
